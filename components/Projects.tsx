...
export default function Projects() {
  ...
  return (
    <section
      id="projects"
      className="relative flex min-h-[calc(100svh-5rem)] scroll-mt-20 flex-col overflow-hidden bg-cream pb-20 pt-12 sm:pb-24 sm:pt-16"
    >
          ...
          >
            {projects.map((project, index) => (
              <motion.button
                ...
                whileHover={{ y: -6 }}
                className="group relative flex aspect-[4/3] w-full shrink-0 snap-start items-center justify-center overflow-hidden rounded-4xl border border-wine/15 bg-bone p-7 text-center shadow-softer transition-colors duration-500 hover:border-wine/35 hover:bg-blush hover:shadow-lift focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-wine focus-visible:ring-offset-4 focus-visible:ring-offset-cream sm:basis-[calc((100%-1.5rem)/2)] sm:p-8 lg:basis-[calc((100%-3rem)/3)]"
              >
                <span
                  aria-hidden="true"
                  className="absolute right-7 top-5 font-display text-5xl text-wine/[0.08] transition-colors duration-300 group-hover:text-wine/[0.13]"
                >
                  {String(index + 1).padStart(2, "0")}
                </span>

                <h3 className="relative z-10 max-w-sm font-display text-2xl font-normal leading-tight text-ink transition-colors duration-300 group-hover:text-wine sm:text-3xl">
                  {project.title}
                </h3>

                <span
                  aria-hidden="true"
                  className="absolute bottom-6 right-6 flex h-10 w-10 items-center justify-center rounded-full border border-wine/20 bg-cream/50 text-wine transition-all duration-300 group-hover:border-wine/40 group-hover:bg-wine group-hover:text-cream"
                >
                  <ArrowRight
                    size={17}
